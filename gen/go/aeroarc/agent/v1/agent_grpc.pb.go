...
type AgentGatewayClient interface {
	...
	// These established stream envelope names intentionally describe direction.
	// buf:lint:ignore RPC_REQUEST_STANDARD_NAME
	// buf:lint:ignore RPC_RESPONSE_STANDARD_NAME
	TelemetryStream(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[AgentStreamMessage, RelayStreamMessage], error)
}
...
func (c *agentGatewayClient) TelemetryStream(ctx context.Context, opts ...grpc.CallOption) (grpc.BidiStreamingClient[AgentStreamMessage, RelayStreamMessage], error) {
	cOpts := append([]grpc.CallOption{grpc.StaticMethod()}, opts...)
	stream, err := c.cc.NewStream(ctx, &AgentGateway_ServiceDesc.Streams[0], AgentGateway_TelemetryStream_FullMethodName, cOpts...)
	if err != nil {
		return nil, err
	}
	x := &grpc.GenericClientStream[AgentStreamMessage, RelayStreamMessage]{ClientStream: stream}
	return x, nil
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AgentGateway_TelemetryStreamClient = grpc.BidiStreamingClient[AgentStreamMessage, RelayStreamMessage]
...
type AgentGatewayServer interface {
	...
	// buf:lint:ignore RPC_REQUEST_STANDARD_NAME
	// buf:lint:ignore RPC_RESPONSE_STANDARD_NAME
	TelemetryStream(grpc.BidiStreamingServer[AgentStreamMessage, RelayStreamMessage]) error
...
func (UnimplementedAgentGatewayServer) TelemetryStream(grpc.BidiStreamingServer[AgentStreamMessage, RelayStreamMessage]) error {
...
func _AgentGateway_TelemetryStream_Handler(srv interface{}, stream grpc.ServerStream) error {
	return srv.(AgentGatewayServer).TelemetryStream(&grpc.GenericServerStream[AgentStreamMessage, RelayStreamMessage]{ServerStream: stream})
}

// This type alias is provided for backwards compatibility with existing code that references the prior non-generic stream type by name.
type AgentGateway_TelemetryStreamServer = grpc.BidiStreamingServer[AgentStreamMessage, RelayStreamMessage]
